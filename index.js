...
app.get("/leaderboard", async (req, res) => {
    ...
    if (type !== "xp" && !type.startsWith("total")) {
        if (duration != "all") {
            var lb =
                await sql`SELECT * from games where EXTRACT(EPOCH FROM (now() - created_at)) < ${duration == "day" ? "86400" : "608400"
        ...
        if (duration != "all") {
            if (type == "xp") {
                var lb =
                    await sql`select name,(sum(coins)+(sum(kills)*300)) as xp from games where verified = true and EXTRACT(EPOCH FROM (now() - created_at)) < ${duration == "day" ? "86400" : "608400"
                        } group by name order by xp desc limit 103`;
            } else {
                var lb =
                    await sql`select name,sum(${sql(type.slice(5))}) as ${sql(type.slice(5))} from games where verified = true and EXTRACT(EPOCH FROM (now() - created_at)) < ${duration == "day" ? "86400" : "608400"
...
app.get("/:user", async (req, res, next) => {
    ...
    */
		...
		(
		select distinct(created_at::date) as Dt from games where created_at >= ${dbuser[0].created_at
            }::date-1 
    ...
    for (var player of Object.values(PlayerList.players)) {
        if (player && !player.ai) {
            ...
            if (socket) {
                ...
                socket.disconnect();
                await sql`INSERT INTO games (name, coins, kills, time, verified) VALUES (${player.name
                    }, ${player.coins}, ${player.kills}, ${Date.now() - player.joinTime}, ${player.verified
                    })`;
            }
        }
    }
