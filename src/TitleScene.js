      ...
      ping("us2").then(res2 => {
        ...
        ping("eu1").then(res3 => {
          ...
          if (pings.filter(p => p.error).length == pings.length) {
            ...
            var scores = pings.map(p => (p.ping) - (p.info.actualPlayercount ? p.info.actualPlayercount * 30 : 0) + (p.info.lag == "No lag" ? 0 : p.info.lag == "Moderate lag" ? 250 : 1000) + (p.info.playerCount > 20 ? Math.abs(20-p.info.playerCount)*70 : 0)).map((p) => !p ? Infinity : p);
