      ...
      ping("us2").then(res2 => {
        ...
        ping("eu1").then(res3 => {
          ...
          if (pings.filter(p => p.error).length == pings.length) {
            ...
            var scores = pings.map(p => (p.ping) - (p.info.playerCount * 10) + (p.info.lag == "No lag" ? 0 : p.info.lag == "Moderate lag" ? 200 : 500) + (p.info.playerCount > 20 ? Math.abs(20-p.info.playerCount)*50 : 0)).map((p) => !p ? Infinity : p);
